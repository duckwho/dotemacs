;; better-base16-theme.el -- better-base16 themes for emacs

;; Author: Kaleb Elwert <belak@coded.io>
;;         Neil Bhakta
;; Maintainer: Kaleb Elwert <belak@coded.io>
;; Version: 1.1
;; Homepage: https://github.com/belak/better-base16-emacs

;;; Commentary:
;; better-base16-theme is a collection of themes built around the better-base16
;; concept (https://github.com/chriskempson/better-base16).  All themes are
;; generated from the official set of color schemes and the templates
;; which are included in this repo.

;;; Code:

(defvar better-base16-theme-use-shell-colors nil
  "Enable support for better-base16-shell.

Because better-base16-shell mangles the color space, we need to use a
different color translation.  This needs to be specified manually
before the themes are loaded.")


(defvar better-base16-shell-colors
  '(
    ;; bg, fg
    :base0 "black"
    :base1 "black"
    ...
    :base0B "brightyellow"
    :base0C "brightblue "
    :base0D "brightmagenta"
    :base0E "brightcyan"
    :base0F "brightwhite"
    )
  "Better-Base16 colors used when in a terminal and not using better-base16-shell.
...
(defvar better-base16-shell-colors-256
  '(
    ;; bg, fg
    :base0 "black"
    :base1 "black"
  ...
  "Better-Base16 colors used when in a terminal and using better-base16-shell.

These mappings are based on the xresources themes combined with
the better-base16-shell code.  If you're using a different terminal
color scheme, you may want to look for an alternate theme for use
in the terminal.")

(defun better-base16-transform-spec (spec colors)
      ...
      (let* ((key       (car  spec))
        ...
        (cond
         ((and (memq key '(:box :underline)) (listp value))
          (setq output (append output (list key (better-base16-transform-spec value colors)))))
         (color
...
(defun better-base16-transform-face (spec colors)
  "Transform a face `SPEC' into an Emacs theme face definition using `COLORS'."
  (let* ((face         (car spec))
         (definition   (cdr spec))
         (shell-colors (if better-base16-theme-use-shell-colors better-base16-shell-colors-256 better-base16-shell-colors)))

    (list face `((((type graphic)) ,(better-base16-transform-spec definition colors))
                 (t                ,(better-base16-transform-spec definition shell-colors))))))

(defun better-base16-set-faces (theme-name colors faces)
  ...
  (apply 'custom-theme-set-faces theme-name
         (mapcar #'(lambda (face)
                     (better-base16-transform-face face colors))
                 faces)))

(defun better-base16-theme-define (theme-name theme-colors)
  "Define the faces for a better-base16 colorscheme given a `THEME-NAME' and a plist of `THEME-COLORS'."
  (better-base16-set-faces
   theme-name
   theme-colors
...
;;; Built-in
;;;; basic colors
     (border                                       :background base0)
     (cursor                                       :background base6)
     (default                                      :foreground base3  :background base0)
     (fringe                                       :background base0)
     (gui-element                                  :background base0)
...
;;;; font-lock
     ...
     (font-lock-comment-delimiter-face             :foreground base7)
...
;;;; mode-line
     (mode-line                                    :foreground base3  :background base2  :box nil)
     ...
     (mode-line-inactive                           :foreground base03 :background base00 :box nil)
...
;;;; company-mode
     (company-tooltip                              :foreground base1 :background base4 :inherit default :inherit fixed-pitch)
     ;; (region                                       :foreground base1 :background base4)
     ...
     (company-tooltip-selection                    :background base2 :inherit font-lock-function-name-face)
...
;;;; flycheck-mode
     (flycheck-error                               :underline (:style wave :color base09))
     (flycheck-info                                :underline (:style wave :color base0A))
     (flycheck-warning                             :underline (:style wave :color base0B))
     (flycheck-fringe-error                        :foreground base09)
     (flycheck-fringe-info                         :foreground base0A)
     (flycheck-fringe-warning                      :foreground base0B)

;;;; flymake-mode
     (flymake-warnline                             :background base0B :underline base0B)
     (flymake-errline                              :background base09 :underline base09)

;;;; flyspell-mode
     (flyspell-duplicate                           :underline (:style wave :color base0B))
     (flyspell-incorrect                           :underline (:style wave :color base09))
...
;;;; stripe-buffer
     (stripe-highlight                             :background base4)
     (stripe-hl-line                               :background base4 :foreground base3)
...
;;;; hl-line-mode
     ...
     (col-highlight                                :background base4)
...
;;;; ivy-mode
     (ivy-current-match                            :background base4 :foreground base5)
...
;;;; linum-mode
     (linum                                        :foreground base5 :background base0)
...
;;;; org-mode
     ...
     (org-block                                    :foreground base3  :background base4 :inherit fixed-pitch :weight bold)
     (org-block-begin-line                         :foreground base3  :background base2 :slant italic :weight bold)
     (org-block-end-line                           :foreground base3  :background base2 :slant italic :weight bold)
     (org-code                                     :foreground base0A :background base2)
     ...
     (org-special-keyword                          :foreground base7)
     (org-table                                    :foreground base0C :background base2)
     (org-todo                                     :foreground base09 :background base01)
     (org-done                                     :foreground base0A :background base02)
     (org-upcoming-deadline                        :foreground base09)
     (org-warning                                  :foreground base01 :weight bold)
     (org-level-1                                  :foreground base0D :background base2)
     (org-level-2                                  :foreground base09 :background base2)
     (org-level-3                                  :foreground base04 :background base2)
     (org-level-4                                  :foreground base05 :background base2)
     (org-level-5                                  :foreground base06 :background base2)
     (org-level-6                                  :foreground base0B :background base2)
     (org-level-7                                  :foreground base0C :background base2)
     (org-level-8                                  :foreground base03 :background base2)
...
;;;; show-paren-mode
     (show-paren-match                             :foreground base08 :background base0A)
...
;;;; term and ansi-term
     (term                                         :foreground base1  :background base0)
     (term-color-black                             :foreground base08 :background base00)
     (term-color-white                             :foreground base0F :background base07)
     (term-color-red                               :foreground base09 :background base01)
     (term-color-yellow                            :foreground base0B :background base03)
     (term-color-green                             :foreground base0A :background base02)
     (term-color-cyan                              :foreground base0E :background base0C)
     (term-color-blue                              :foreground base0C :background base04)
     (term-color-magenta                           :foreground base0D :background base0E)
...
(provide 'better-base16-theme)

;;; better-base16-theme.el ends here
