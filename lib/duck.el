;;; duck.el --- Duck for Emacs.

;; Copyright (C) 2017 Ean Chan Park

;; Author: Ean Chan Park <theduckwho@gmail.com>
;; URL: http://github.com/duckwho/emacs-duck-theme
;; Version: 0.1
...
;;; Installation:
;;
;;   Drop the `duck-theme.el` somewhere in your `load-path` and
...
(defcustom duck-distinct-fringe-background nil
  "Make the fringe background different from the normal background color.
Also affects `linum-mode' background."
  :type 'boolean
  :group 'duck)

(defcustom duck-distinct-doc-face nil
  "Make `font-lock-doc-face' stand out more.
Related discussion: https://github.com/bbatsov/duck-emacs/issues/158"
  :type 'boolean
  :group 'duck)

(defcustom duck-use-variable-pitch t
  "Use variable pitch face for some headings and titles."
  :type 'boolean
  :group 'duck)

(defcustom duck-use-less-bold nil
  "Use bold weight less often."
  :type 'boolean
  :group 'duck)

(defcustom duck-use-more-italic nil
  "Use italic slant more often."
  :type 'boolean
  :group 'duck)

(defcustom duck-emphasize-indicators t
  "Use more colors for indicators such as git:gutter, flycheck and similar."
  :type 'boolean
  :group 'duck)

(defcustom duck-high-contrast-mode-line nil
  "Make the active/inactive mode line stand out more."
  :type 'boolean
  :group 'duck)

(defcustom duck-height-minus-1 0.8
  "Font size -1."
  :type 'number
  :group 'duck)

(defcustom duck-height-plus-1 1.1
  "Font size +1."
  :type 'number
  :group 'duck)

(defcustom duck-height-plus-2 1.15
  "Font size +2."
  :type 'number
  :group 'duck)

(defcustom duck-height-plus-3 1.2
  "Font size +3."
  :type 'number
  :group 'duck)

(defcustom duck-height-plus-4 1.3
  "Font size +4."
  :type 'number
  :group 'duck)

(defcustom duck-scale-org-headlines t
...
;;;###autoload
(defun duck-color-blend (color1 color2 alpha)
...
;;; Setup Start
(defmacro duck-with-color-variables (variant &rest body)
  (declare (indent 0))
  `(let* ((class '((class color) (min-colors 89)))
         ...
         ;; Duck palette names, use these instead of -fg -bg...
         ...
         (s-maybe-bold (if duck-use-less-bold
                           'unspecified 'bold))
         (s-maybe-italic (if duck-use-more-italic
                             'italic 'normal))
         (s-variable-pitch (if duck-use-variable-pitch
                               'variable-pitch 'default))
         (s-fringe-bg (if duck-distinct-fringe-background
                          base02 base03))
         (s-fringe-fg base01)

         (s-header-line-fg (if duck-high-contrast-mode-line
                               base1 base0))
         (s-header-line-bg (if duck-high-contrast-mode-line
                               base02 base03))
         (s-header-line-underline (if duck-high-contrast-mode-line
                                      nil base02))

         (s-mode-line-fg (if duck-high-contrast-mode-line
                             base03 base0))
         (s-mode-line-bg (if duck-high-contrast-mode-line
                             base0 base02))
         (s-mode-line-underline (if duck-high-contrast-mode-line
                                    nil s-line))

         (s-mode-line-buffer-id-fg (if duck-high-contrast-mode-line
                                       'unspecified base1))
         (s-mode-line-inactive-fg (if duck-high-contrast-mode-line
                                      base0 base01))
         (s-mode-line-inactive-bg (if duck-high-contrast-mode-line
                                      base02 base03))
         (s-mode-line-inactive-bc (if duck-high-contrast-mode-line
                                      base02 base02))
         )
     ,@body))

(defun create-duck-theme (variant theme-name &optional childtheme)
...
;;; Color palette
  (duck-with-color-variables variant
;;; Theme Faces
...
;;;;; font lock
     ...
     `(font-lock-doc-face ((,class (:foreground ,(if duck-distinct-doc-face violet cyan)
                                                :slant ,s-maybe-italic))))
...
;;;;; auctex
     ...
     `(font-latex-slide-title-face ((,class (:inherit (,s-variable-pitch font-lock-type-face)
                                                      :weight bold :height ,duck-height-plus-3))))
     `(font-latex-string-face ((,class (:foreground ,cyan))))
     `(font-latex-subscript-face ((,class (:height ,duck-height-minus-1))))
     `(font-latex-superscript-face ((,class (:height ,duck-height-minus-1))))
...
;;;;; calfw
     ...
     `(cfw:face-title ((,class (:inherit ,s-variable-pitch :foreground ,yellow
                                         :weight bold :height ,duck-height-plus-4))))
...
;;;;; custom
     `(custom-face-tag ((,class (:inherit ,s-variable-pitch :height ,duck-height-plus-3
                                          :foreground ,violet :weight normal))))
     `(custom-variable-tag ((,class (:inherit ,s-variable-pitch
                                              :foreground ,cyan :height ,duck-height-plus-3))))
     `(custom-comment-tag ((,class (:foreground ,base01))))
     `(custom-group-tag ((,class (:inherit ,s-variable-pitch :foreground ,blue :height ,duck-height-plus-3))))
     `(custom-group-tag-1 ((,class (:inherit ,s-variable-pitch :foreground ,red :height ,duck-height-plus-3))))
...
;;;;; diff
     ...
     `(diff-refine-added
       ((((class color) (background light))
         (:background ,(duck-color-blend "#ddffdd" green 0.7)))
        (((class color) (background dark))
         (:background ,(duck-color-blend "#446644" green 0.7)))))
     `(diff-refine-changed
       ((((class color) (background light))
         (:background ,(duck-color-blend "#ddddff" blue 0.7)))
        (((class color) (background dark))
         (:background ,(duck-color-blend "#444466" blue 0.7)))))
     `(diff-refine-removed
       ((((class color) (background light))
         (:background ,(duck-color-blend "#ffdddd" red 0.7)))
        (((class color) (background dark))
         (:background ,(duck-color-blend "#664444" red 0.7)))))
...
;;;;;; alternative ediff (not finished)
     ...
     ;;                               :background ,(duck-color-blend blue base03 0.25))
     ;;                              )))
     ;; `(ediff-fine-diff-B ((,class (
     ;;                               :background ,(duck-color-blend violet base03 0.25))
     ;;                              )))
     ;; `(ediff-fine-diff-C ((,class (
     ;;                               :background ,(duck-color-blend yellow base03 0.25))
     ;;                              )))
     ;; `(ediff-current-diff-A ((,class (
     ;;                                  :background ,(duck-color-blend blue base03 0.15)
     ;;                                              ))))
     ;; `(ediff-current-diff-B ((,class (
     ;;                                   :background ,(duck-color-blend violet base03 0.15)
     ;;                                              ))))
     ;; `(ediff-current-diff-C ((,class (
     ;;                                  :background ,(duck-color-blend yellow base03 0.15)
     ;;                                              ))))
     ;; `(ediff-even-diff-A ((,class (
     ;;                                ;; :background ,(duck-color-blend base0 base03 0.15)
     ;;                               :background ,base02
     ;;                               ;; :foreground ,base2
     ;;                                ;; :background ,(duck-color-blend green base02 0.15)
     ;;                                           ))))
...
;;;;; flycheck
     ...
     `(flycheck-info
       ((,(append '((supports :underline (:style wave))) class)
         (:underline (:style wave :color ,(if duck-emphasize-indicators
                                              blue base03)) :inherit unspecified))
        (,class (:foreground ,blue-hc :background ,blue-lc :weight bold :underline t))))
     `(flycheck-fringe-error
       ((,class (:foreground ,(if duck-emphasize-indicators
                                  red-hc red)
                             :background ,(if duck-emphasize-indicators
...
;;;;; git-gutter
     `(git-gutter:added
         ((,class (:weight normal
                           :foreground ,(if duck-emphasize-indicators
                                            green s-fringe-fg)
...
;;;;;; go-mode
     ...
     `(go-coverage-1 ((,class (:foreground ,(duck-color-blend blue yellow (/ 2.0 6))))))
     `(go-coverage-2 ((,class (:foreground ,(duck-color-blend blue yellow (/ 3.0 6))))))
     `(go-coverage-3 ((,class (:foreground ,(duck-color-blend blue yellow (/ 4.0 6))))))
     `(go-coverage-4 ((,class (:foreground ,(duck-color-blend blue yellow (/ 5.0 6))))))
     `(go-coverage-5 ((,class (:foreground ,blue))))
     `(go-coverage-6 ((,class (:foreground ,(duck-color-blend cyan blue (/ 2.0 6))))))
     `(go-coverage-7 ((,class (:foreground ,(duck-color-blend cyan blue (/ 3.0 6))))))
     `(go-coverage-8 ((,class (:foreground ,(duck-color-blend cyan blue (/ 4.0 6))))))
     `(go-coverage-9 ((,class (:foreground ,(duck-color-blend cyan blue (/ 5.0 6))))))
...
;;;;; hi-lock-mode
     `(hi-yellow ((,class (:foreground ,(duck-color-blend yellow base1 0.5)
                                       :background,(duck-color-blend yellow base03 0.15)))))
     `(hi-pink ((,class (:foreground ,(duck-color-blend magenta base1 0.5)
                                       :background,(duck-color-blend magenta base03 0.15)))))
     `(hi-green ((,class (:foreground ,(duck-color-blend green base1 0.5)
                                       :background,(duck-color-blend green base03 0.15)))))
     `(hi-blue ((,class (:foreground ,(duck-color-blend blue base1 0.5)
                                       :background,(duck-color-blend blue base03 0.15)))))
     `(hi-black-b ((,class (:foreground ,base1
                                        :background ,base03
                                        :weight bold))))
     `(hi-blue-b ((,class (:weight bold
                                   :foreground ,(duck-color-blend cyan base1 0.7)
                                   :background ,(duck-color-blend cyan base03 0.2)))))
     `(hi-green-b ((,class (:weight bold
                           :foreground ,(duck-color-blend green base1 0.7)
                           :background ,(duck-color-blend green base03 0.2)))))
     `(hi-red-b ((,class (:weight bold
                                  :foreground ,(duck-color-blend red base1 0.7)
                                  :background ,(duck-color-blend red base03 0.2)))))
...
;;;;;; headings and diffs
     ...
     `(magit-diff-hunk-heading
       ((t (:background ,(duck-color-blend yellow base03 0.1)))))
     `(magit-diff-hunk-heading-highlight
       ((t (:background ,(duck-color-blend yellow base02 0.1)))))
...
;;;;; org-mode
     `(org-agenda-structure
       ((,class (:foreground ,base1 :background ,base02
                             :weight bold :slant normal :inverse-video nil :height ,duck-height-plus-1
                             :underline nil
     ...
     `(org-document-title ((,class (:foreground ,base1  :weight bold :height ,duck-height-plus-4))))
...
;;;;; outline
     `(outline-1 ((,class (:inherit org-level-1 :height ,duck-height-plus-4))))
     `(outline-2 ((,class (:inherit org-level-2 :height ,duck-height-plus-3))))
     `(outline-3 ((,class (:inherit org-level-3 :height ,duck-height-plus-2))))
     `(outline-4 ((,class (:inherit org-level-4 :height ,duck-height-plus-1))))
...
;;;;; powerline
     `(powerline-active1 ((,class ,(if duck-high-contrast-mode-line
                                       `(:background ,base00 :foreground ,base03)
                                       `(:background ,base03 :foreground ,base00)))))
     `(powerline-active2 ((,class ,(if duck-high-contrast-mode-line
                                       `(:background ,base01 :foreground ,base03)
                                       `(:background ,base02 :foreground ,base00)))))
     `(powerline-inactive1 ((,class ,(if duck-high-contrast-mode-line
                                         `(:background ,base03 :foreground ,base1)
                                         `(:background ,base02 :foreground ,base01)))))
     `(powerline-inactive2 ((,class ,(if duck-high-contrast-mode-line
...
;;;;; smart-mode-line
     ;; use (setq sml/theme nil) to enable Duck for sml
...
;;;;; sx (WIP)
     ...
     ;; ensures readability as a bg for all duck faces. If it's possible, that is.
...
;;;;;; headings
     `(sr-active-path-face ((,class (:background ,blue :foreground ,base03
                                                 :height ,duck-height-plus-1  :weight bold))))
     `(sr-editing-path-face ((,class (:background ,yellow :foreground ,base03
                                                  :weight bold :height ,duck-height-plus-1))))
...
;;;;; weather-metno
     `(weather-metno-date ((,class (:foreground ,yellow :height ,duck-height-plus-3))))
...
;;;;; highlight-symbol
     ...
     `(highlight-symbol-colors
       (--map (duck-color-blend it ,base03 0.25)
...
;;;;; smartrep
     `(smartrep-mode-line-active-bg (duck-color-blend ,green ,s-mode-line-bg 0.2))
;;;;; term
...
;;;;; vc
     ...
     `(vc-annotate-color-map
       '((20 . ,red)
         (40 . ,(duck-color-blend yellow red (/ 2.0 4)))
         (60 . ,(duck-color-blend yellow red (/ 3.0 4)))
         (80 . ,yellow)
         (100 . ,(duck-color-blend green yellow (/ 2.0 6)))
         (120 . ,(duck-color-blend green yellow (/ 3.0 6)))
         (140 . ,(duck-color-blend green yellow (/ 4.0 6)))
         (160 . ,(duck-color-blend green yellow (/ 5.0 6)))
         (180 . ,green)
         (200 . ,(duck-color-blend cyan green (/ 2.0 6)))
         (220 . ,(duck-color-blend cyan green (/ 3.0 6)))
         (240 . ,(duck-color-blend cyan green (/ 4.0 6)))
         (260 . ,(duck-color-blend cyan green (/ 5.0 6)))
         (280 . ,cyan)
         (300 . ,(duck-color-blend blue cyan (/ 2.0 5)))
         (320 . ,(duck-color-blend blue cyan (/ 3.0 5)))
         (340 . ,(duck-color-blend blue cyan (/ 4.0 5)))
         (360 . ,blue)))
